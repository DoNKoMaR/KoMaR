...
async function reload(conn, pluginFolder = pluginFolder, pluginFilter = pluginFilter, _ev, filename) {
    if (pluginFilter(filename)) {
        ...
        if (filename in plugins) {
            if (existsSync(dir)) conn.logger.info(` Plugin - '${filename}' actualizado correctamente`)
            else {
                conn?.logger.warn(`Plugin eliminado - '${filename}'`)
                return delete plugins[filename]
            }
        } else conn?.logger.info(`Plugin nuevo - '${filename}'`)
        ...
        if (err) conn.logger.error(`Error de sintaxis al cargar '${filename}'\n${format(err)}`)
            ...
            conn?.logger.error(`Error se necesita el plugin '${filename}\n${format(e)}'`)
        } finally {
